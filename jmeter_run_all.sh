...
ENDSSH
	...
	echo "************************"
	cp $carbon_home/repository/logs/gc.log gc_log_$scenario$concurrency.log
	sar -q > $(date +`hostname`-%d-%m-%y-%H%M_log_$scenario$concurrency.log)
        echo Killing All Carbon Servers......
        killall java
	exit
ENDSSH2
